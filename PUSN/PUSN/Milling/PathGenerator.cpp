...
vector<Vector3> PathGenerator::GenerateSurfaceIntersectionPaths()
{
	...
	//qwe
	//tmp = GenerateUnrestrictedPath(model.GetTail(), model.GetTorso(), Vector3(-40, 10, minZ+10));
	...
	//tmp.erase(tmp.begin(), tmp.begin()+891);
	//AddSafe(tmp);
	//Append(result, tmp);
...
vector<Vector3> PathGenerator::GenerateSurfacePaths()
{
	...
	tmp = GenerateUnrestrictedPath(model.GetTorso(), model.GetRightEar(), Vector3(20, 20, minZ + 10));
	tmp.rbegin()->y += 10;
	tmp[0].x += 10;
	TrimEnd3(tmp3, tmp);
	TrimStart3(tmp3, tmp);
